...
public class StatisticCalculation implements Function<List<Ick>, Integer> {
	...
	public Integer apply(List<Ick> Icks){
		...
		int newIcksThisMonth = Icks.stream().filter(ick -> ick.getCreatedOn().atZone(ZoneId.systemDefault()).getMonth() == currentMonth).toArray().length;
		int oldIcks = Icks.stream().filter(ick -> ick.getCreatedOn().atZone(ZoneId.systemDefault()).getMonth() != currentMonth).toArray().length;
